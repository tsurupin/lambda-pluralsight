...
package object functions {
  ...
  def rddToRDDActivity(input: RDD[String]) = {
    ...
    input.mapPartitionsWithIndex( { (index, it) =>
      ...
      it.flatMap { kv =>
       ...
       val record = line.split("\t")
        ...
        if (record.length == 7) {
          Some(Activity(record(0), toLong / MS_IN_HOUR * MS_IN_HOUR, record(1), record(2), record(3), record(4), record(5), record(6), Map("topic" -> or.topic, "kafkaPartition" -> or.partition, "fromOffset" -> or.fromOffset.toString, "untilOffset" -> or.untilOffset.ToString)))
        } else {
          None
        }
      }
  ...
  def mapActivityStateFunc = (k: (String: Long) v: Option(ActivityByProduct), state: State[(Long, Long, Long)]) => {
    var (purchase_count, add_to_count, page_view_count) = state.getOption().getOrElse((0L, 0L, 0L))
    ...
    state.udate((purchase_count, add_to_count, page_view_count))
  ...
  def mapVisitorsStateFunc = (k: (String, Long), v: Option(HLL), state: State(HLL)) => {
    ...
    val newVisitorHLL = v match {
      case Some(visitorHLL) => currentVisitorHLL + visotorHLL
